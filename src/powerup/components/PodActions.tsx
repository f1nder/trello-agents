...
const PodActions = ({ pod, onStop, onStreamLogs, disabled = false, isStopping = false, variant = 'default' }: PodActionsProps) => {
    ...
    return (
      <div style={{ display: 'flex', gap: '0.35rem' }}>
        <button
          ...
          title="Stop pod"
        >
          {isStopping ? <span className="icon-button__spinner" aria-hidden="true" /> : <IconStop />}
        </button>
        <button
          type="button"
          className="icon-button"
